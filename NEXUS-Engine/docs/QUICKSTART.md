...
| «Модель ничего не выучила» | смотрите строку «токенов на параметр» в начале обучения: при значении < 1 данных не хватает. Увеличьте `--math`/`--samples` или возьмите `--preset tiny` |
| Обучение закончилось раньше `--max-steps` | было в ревизиях до `e387a19`: бюджет обрывался на первой эпохе. Обновитесь |
